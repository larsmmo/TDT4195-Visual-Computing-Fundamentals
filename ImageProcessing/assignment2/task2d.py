...
for i, idx in enumerate(indices):
    plt.subplot(2, 5, i + 1)
    im_ker = torch_image_to_numpy(first_conv_layer.weight[idx, ...])
    plt.imshow(im_ker)
    plt.subplot(2, 5, i + 6)
    im_act = torch_image_to_numpy(activation[0, idx, :, :])
    plt.imshow(im_act, cmap = "gray")
    
plt.savefig('Task2d_visualization.png', format = "png")
### END YOUR CODE HERE ### 
