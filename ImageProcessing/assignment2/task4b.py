...
def convolve_im(im: np.array,
    ...
    if verbose:
        ...
        plt.subplot(1, 5, 2) 
        plt.imshow(np.fft.fftshift(np.log(np.abs(im_fft))), cmap="gray")

        H, W = im.shape
        K_H, K_W = kernel.shape
        kernel = np.pad(kernel, ((0, H-K_H), (0, H-K_H)), mode = 'constant')           #Padding the kernel

        fft_kernel = np.abs(np.fft.fft2(kernel))
        plt.subplot(1, 5, 3) 
        plt.imshow(np.fft.fftshift(fft_kernel), cmap="gray")

        im_fft_conv = im_fft * fft_kernel
        plt.subplot(1, 5, 4) 
        plt.imshow(np.fft.fftshift(np.log(np.abs(im_fft_conv))), cmap = "gray")
