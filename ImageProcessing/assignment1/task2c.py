...
def convolve_im(im, kernel):
    ...
    im_zero_padded = np.zeros((H+2 * (K//2), W + 2 * (K//2), 3))
    im_zero_padded[(K//2):-(K//2), (K//2):-(K//2), :] = im
    ...
    for i in range(H):
        for j in range(W):
            for c in range(3):
                p = kernel_flipped * im_zero_padded[i:i+K, j:j+K, c]
                im[i, j, c] = p.sum();
...
if __name__ == "__main__":
    ...
    smoothed_im2 = convolve_im(im.copy(), h_b)      # Added im.copy()
