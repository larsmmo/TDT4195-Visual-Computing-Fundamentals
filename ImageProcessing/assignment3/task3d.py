...
def fill_holes(im: np.ndarray, starting_points: list, num_iterations: int) -> np.ndarray:
    ...
    result = np.zeros_like(im, dtype=bool)

    for row, column in starting_points:
        result[row,column] = 1

    for k in range(1, num_iterations + 1):
        result = np.logical_and(skimage.morphology.binary_dilation(result, selem=structuring_element),np.invert(im))

    return np.logical_or(im, result)
    ### END YOUR CODE HERE ### 
